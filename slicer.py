...
def on_select(shapes):
    ...
    if s.ShapeType() == TopAbs_EDGE:
        ...
        print(f'Edge type: {tmp.GetType().name}')
        ...
        if tmp.GetType() == GeomAbs_Line:
            ...
            print("circle parameters: %.6f, %.6f" %
                  (tmp.FirstParameter() / math.pi, tmp.LastParameter() / math.pi))
...
def wire_gcode(wire: TopoDS_Wire):
    print(f'Wire Orientation: {wire.Orientation().name}')
